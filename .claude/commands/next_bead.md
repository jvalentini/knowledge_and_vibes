...
1. Did you run `/reflect` to verify completion?
2. Did you run `/commit` to commit changes?
3. Close the bead:
   ```bash
...
**Note:** Don't use `bd sync` - it fails in multi-agent environments due to HEAD ref conflicts. Instead, include `.beads/` in your regular git commits.
...
```
mcp__mcp-agent-mail__call_extended_tool(
...
mcp__mcp-agent-mail__call_extended_tool(tool_name="file_reservation_status", ...)

# Check other agents
ReadMcpResourceTool(server="mcp-agent-mail", uri="resource://agents/PROJECT_SLUG")

# Send [CLAIMED] announcement
mcp__mcp-agent-mail__send_message(..., subject="[CLAIMED] <task-id> - Title", ...)

# When done, send [CLOSED] announcement
mcp__mcp-agent-mail__send_message(..., subject="[CLOSED] <task-id> - Title", ...)
```
